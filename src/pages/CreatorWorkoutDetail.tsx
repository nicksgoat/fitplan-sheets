...
const CreatorWorkoutDetail = () => {
    ...
    const getWorkoutBySlug = async () => {
      try {
        const {
          data: profileData,
          error: profileError
        } = await supabase.from('profiles').select('id, username, display_name').eq('username', username).maybeSingle();
        if (profileError) {
        ...
        if (profileData.username && profileData.username !== username) {
          ...
          navigate(buildCreatorProductUrl(profileData.username, workoutSlug), {
            replace: true
          });
          return;
        }
        const {
          ...
          `).eq('slug', workoutSlug).eq('weeks.programs.user_id', profileData.id).maybeSingle();
        if (workoutError) {
  ...
  const getInitials = () => {
    if (creatorName) {
      return creatorName.split(' ').map(n => n[0]).join('').toUpperCase().substring(0, 2);
    }
      ...
      <Card className="bg-dark-200 border-dark-300 mb-4">
        <CardContent className="p-4 flex items-center">
          ...
          <div className="flex-1">
            ...
            {creatorProfile?.bio && <p className="text-gray-400 text-sm line-clamp-1 mt-1">{creatorProfile.bio}</p>}
          </div>
        </CardContent>
      </Card>
