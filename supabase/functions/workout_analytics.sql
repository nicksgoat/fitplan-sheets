...
CREATE OR REPLACE FUNCTION public.get_workout_log_counts(p_user_id UUID)
RETURNS TABLE (
  log_date DATE,
...
CREATE OR REPLACE FUNCTION public.get_most_used_exercises(p_user_id UUID, p_limit INT)
RETURNS TABLE (
  exercise_name TEXT,
  usage_count BIGINT
) LANGUAGE SQL SECURITY DEFINER AS $$
...
CREATE OR REPLACE FUNCTION public.calculate_workout_streak(p_user_id UUID)
RETURNS TABLE (
